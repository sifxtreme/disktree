...
| Clean up loads under heavy load | harness step `clean up` (suggestions within 45 s); `curl -w %{time_total} 127.0.0.1:7321/api/h/local/suggest` | FAILING 2026-09-25 at load 80–130: `/suggest` took 5–33 s, and the app's 30 s request timeout loses the slow ones. At load 14 it takes 0.06–0.19 s, so this is CPU starvation (disk-web runs at Background priority). Deferred. Fix when wanted: (1) cache `/suggest` per snapshot, because it depends only on the snapshot (`now = meta.taken_at`), which changes hourly; (2) run com.asif.disk-web at ProcessType Standard, as disk-mem does. |
